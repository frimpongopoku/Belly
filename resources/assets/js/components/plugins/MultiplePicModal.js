...
class MultiplePicModal extends React.Component{
	constructor(props){
		super(props);
	...
	multButtonClick(){
		let phase = $(".multiple-m-pages-btn").attr('data-phase'); 
		if(phase ==='toggled'){
			$('.multiple-m-pages-btn').removeClass('multiple-side-active z-depth-2'); 
			$('.multiple-m-pages-btn').attr('data-phase','untoggled')
		}else{
			$('.multiple-m-pages-btn').addClass('multiple-side-active z-depth-2');
			$('.multiple-m-pages-btn').attr('data-phase','toggled')
		}
	}
	extrasZoom(ID, inArrayID){
		var zoomed = $('#pic-piece-body-'+ID+"-m-"+inArrayID).attr('data-zoomed'); 
		if (zoomed=== "false"){
			$('#pic-piece-body-'+ID+"-m-"+inArrayID)
			.css({
				position:'relative',
				zIndex:1,
				transform:'scale(1.3)',
			...
			$('#pic-piece-body-'+ID+"-m-"+inArrayID).attr('data-zoomed','true');			
		}else if(zoomed="true"){
			$('#pic-piece-body-'+ID+"-m-"+inArrayID)
			.css({
				position:'relative',
				zIndex:0,
				transform:'scale(1)',
				borderRadius:0, transition:'0.2s ease-in all'
			}); 
			$('#pic-piece-body-'+ID+"-m-"+inArrayID).attr('data-zoomed','false');	
		}
	}
	zoom(ID){
	...
	spillExtraImages(extraImgList){
		//takes a list of image links as an array and spills a div with approppriate classes and IDs... reated to the parent_id,
		// and the item's posiion-1 in the array
		return extraImgList.map( ( img, index )=>{ 
			return (
					<div data-zoomed="false" 
					onClick={()=>{ this.extrasZoom(this.props.piece_id,index)}}
						key ={ index } 
						id = {'pic-piece-body-'+this.props.piece_id+"-m-"+index}
	                    className ='pic-piece-image extra-img-div' 
	                 >
	                 </div>
				);
		})
	...
	deletePicture(){
		//close modal, delete item, and then update the state with the item removed!
		$('.modal .close').click();
