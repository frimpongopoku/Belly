...
class UploadController extends Controller
{
  ...
  public function pdfUpload($content,$title,$paper_term,$subcourse){
    if(count($content) > 1){
      $type = "more";
    }else{
      $type="single";
    ...
    foreach ($content as $file) {
      $num ++;
      $redifinedTitle = $title." ".(string)$num;
      $results = $this->checkFile($file);
      $strippedTitle = str_replace(' ','-',$title); 
      $newFileName = "QB-".$strippedTitle.'-'.uniqid().'-USER-ID-'.Auth::user()->id.'-timely-PDF-'.time().'.'.$results['info']['ext'];
      $path = "users/PDFs/";
      if($results['status']=="pass"){
        if( $this->uploadFile($file,$path,$newFileName) =="true"){
          if($type=="more"){
            $this->saveUserPDFDetails($redifinedTitle,$path.$newFileName,$paper_term,$subcourse);
          }else{
               $this->saveUserPDFDetails($title,$path.$newFileName,$paper_term,$subcourse);
          }
        }
      }
    }
  ...
  public function saveUserPDFDetails($title,$path,$paper_term,$subcourse){
    $new = new PdfPiece(); 
    $new->title = $title; 
    $new->user_id = Auth::user()->id; 
    $new->pdf_link = $path; 
    $new->course = Auth::user()->course;
    $new->paper_term = $paper_term;
    $new->name = Auth::user()->name; 
    $new->save();
    $this->addSubCourse($new->id,'pdf',$subcourse);
  }
    ...
    $this->pdfUpload($PDFArray,$description,$request->pic_paper_term,$request->subcourse);
    $firstImageCheck= count($imageArray) != 0 ? $this->checkFile($imageArray[0]):['status'=>'empty'];
    if($firstImageCheck['status']=="pass"){
      $picture = new PicturePiece();
      $picture->name = Auth::user()->name; 
      $picture->user_id = Auth::user()->id; 
      $picture->course = Auth::user()->course;
      $picture->paper_term = $request->pic_paper_term;
      ...
      if(count($imageArray) >1){
        ...
        if($picture->save()){
          $this->addSubCourse($picture->id,'image',$request->subcourse);
          return $picture;
        }
        else{
